...
*   **`index.html`**:
    *   *Ý nghĩa:* File HTML duy nhất chứa toàn bộ bố cục cấu trúc ứng dụng. Được chia làm 4 khu vực tương ứng với 4 tab làm việc.
*   **`css/style.css`**:
...
*   **`js/app.js`**:
    *   *Ý nghĩa:* Logic điều khiển giao diện: xử lý sự kiện khi click chọn bàn, hiển thị hộp thoại pop-up (Modal), quản lý giỏ hàng tạm thời và hiển thị hóa đơn tính tiền.
...
1.  **Giao diện JS (`app.js` & `api.js`)** gửi yêu cầu tới API `/api/chitiethoadon/them-mon` kèm theo thông tin `Size L`.
